...
export default function MemoryGameSetupPage() {
  ...
  const handleNumPlayersChange = (value: number[]) => {
    ...
    setPlayerNames((prevNames) => {
      ...
      while (newNames.length < newNumPlayers) {
        newNames.push(`${t('player')} ${newNames.length + 1}`)
      }
  ...
  const handleNumCardPairsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(value)) {
      if (value < 2 || value > 25) {
        setNumCardPairsError(t('numPairsError'));
      } else {
  ...
  <Card className={`w-full max-w-2xl border ${isChristmas ? 'border-red-100/30' : 'border-brand-primary-100/30'} bg-white/20 backdrop-blur-lg shadow-lg mb-4 rounded-3xl`}>
        <CardHeader className="text-center pb-6">
          ...
          <CardTitle className="text-3xl font-semibold text-brand-primary-900/90 bg-white/15 px-4 py-2 rounded-2xl backdrop-blur-lg inline-block tracking-tight">{t('memoryGameSetup')}</CardTitle>
        ...
        <CardContent className="space-y-8">
          ...
          <div className="space-y-4">
            <Label className={`${isChristmas ? 'text-red-700' : 'text-brand-primary-900'} text-lg font-semibold drop-shadow-md`}>{t('playerNames')}:</Label>
          ...
          {/* Versão do jogo */}
          <div className="space-y-4">
            <Label className={`${isChristmas ? 'text-red-700' : 'text-brand-primary-900'} text-lg font-semibold drop-shadow-md`}>{t('version')}</Label>
            <div>
              <select
                value={version}
              ...
              >
                <option value="restauracao">{t('restoration')}</option>
                <option value="natal">{t('christmas')}</option>
              </select>
            </div>
          </div>
